<div class="filament-taskboard" wire:id="{{ $this->getId() }}">
    ...
    @php
    $totalTasks = $this->statuses->sum(fn($status) => $status->tasks->count());
    @endphp

    @if($totalTasks === 0)
    <div class="flex items-center justify-center h-full w-full">
        <div class="text-center">
            <p class="text-gray-500 dark:text-gray-400 text-lg font-medium">Новых задач пока нет</p>
        </div>
    </div>
    @else
    <div class="flex gap-4 overflow-x-auto pb-4 h-full">
        ...
        <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 shadow h-full flex flex-col flex-shrink-0"
            style="width: 320px; min-width: 320px;" wire:key="status-wrapper-{{ $status->id }}">
            <h3 class="text-lg font-semibold mb-4">{{ $status->name }}</h3>

            {{-- Added visual border and flex-grow to ensure drop target is large --}}
            <div class="space-y-4 task-column flex-grow min-h-[100px]" id="status-col-{{ $status->id }}"
                data-status-id="{{ $status->id }}" style="border: 2px dashed transparent;">
                @foreach($status->tasks as $task)
                <div @class([ 'rounded-lg p-3 shadow-sm task-card cursor-move group' , 'bg-red-200 dark:bg-red-700'=>
                    $task->isOverdue(),
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ Str::limit($task->description, 150) }}
                    </p>
